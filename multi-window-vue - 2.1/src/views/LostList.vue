  ...
  <div class="list-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div class="header-text">
          <h1 class="header-title">失物信息</h1>
          <p class="header-subtitle">寻找丢失的物品，让它们找到回家的路</p>
        </div>
      </div>
      <div class="header-stats">
        <div class="stat-item">
          <span class="stat-value">{{ pagination.total }}</span>
          <span class="stat-label">条失物记录</span>
        </div>
      </div>
    </div>
    ...
    <div class="toolbar">
      <div class="view-switch">
        ...
        <span class="view-label">当前 {{ lostList.length }} 条记录</span>
      </div>
    ...
    <div class="content-area">
      ...
      <div v-if="viewMode === 'table'">
        ...
        <div v-else-if="lostList.length === 0" class="empty-wrapper">
          <el-empty description="暂无失物信息" :image-size="200">
            <template #description>
              <p class="empty-text">暂时没有找到失物信息</p>
              <p class="empty-tip">您可以尝试其他搜索条件，或发布新的失物信息</p>
            </template>
          </el-empty>
        </div>
        ...
        <template v-else>
          ...
          >
            ...
            <el-table-column prop="location" label="丢失地点" min-width="150" show-overflow-tooltip />
        ...
        >
          <el-card class="item-card" shadow="hover" @click="viewDetail(item.id)">
            <div class="card-image-wrapper">
              <el-image
                :src="item.images && JSON.parse(item.images || '[]').length > 0 ? JSON.parse(item.images)[0] : '/images/default-lost.png'"
                class="card-image"
                fit="cover"
              >
            ...
            <div class="card-content">
              ...
              <div class="card-info-row">
                ...
                <span class="info-text" :title="item.location">{{ item.location || '未知地点' }}</span>
              </div>
              
              <div class="card-info-row">
                <el-icon><Clock /></el-icon>
                <span class="info-text">{{ formatDate(item.publishTime) }}</span>
              </div>
    ...
    <div class="pagination-section">
      <el-pagination
        ...
        :total="pagination.total || lostList.length"
...
import { Plus, Search, RefreshRight, Grid, Menu, Box, View, Picture, Location, Clock } from '@element-plus/icons-vue';
...
export default {
  ...
  setup() {
    ...
    const pageCount = computed(() => {
      const effectiveTotal = Number(pagination.total) || lostList.value.length || 0;
...
.list-page {
  ...
  height: 100%;
  min-height: 0;
  background: radial-gradient(900px 380px at 8% -10%, rgba(37, 99, 235, 0.12), transparent 60%),
