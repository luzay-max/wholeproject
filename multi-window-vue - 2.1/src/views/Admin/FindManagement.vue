  ...
  <div class="admin-page">
    ...
    <div class="table-area">
      <div class="table-header">
        <span class="table-title">招领列表</span>
      ...
      >
        ...
        <el-table-column prop="location" label="发现地点" width="150"></el-table-column>
...
import { getFindItems, updateFindStatus, deleteFindItem, batchDeleteFindItems } from '../../api/adminApi';
  ...
  setup() {
    ...
    const loadFindList = async () => {
      try {
        ...
        const res = await getFindItems(params);
        findList.value = res.data.list || [];
        pagination.total = res.data.total || 0;
      } catch (error) {
        ElMessage.error('加载招领列表失败：' + (error.message || '未知错误'));
      } finally {
        loading.value = false;
      }
    };
    ...
    const handleConfirmStatus = async () => {
      ...
      try {
        ...
        // 调用实际API更新招领状态
        await updateFindStatus(statusForm.id, {
    ...
    const handleDelete = async (item) => {
      try {
        ...
        // 调用实际API删除招领
        await deleteFindItem(item.id);
        
        ElMessage.success('招领信息删除成功');
        loadFindList();
      } catch (error) {
        if (error !== 'cancel') {
    ...
    const handleBatchDelete = async () => {
      if (selectedItems.value.length === 0) {
        ElMessage.warning('请选择要删除的招领信息');
        return;
      }
      
      try {
        await ElMessageBox.confirm(
          `确认要删除选中的 ${selectedItems.value.length} 条招领信息吗？`,
          '批量删除确认',
          {
            confirmButtonText: '确认',
            cancelButtonText: '取消',
        ...
        await batchDeleteFindItems(idsToDelete);
