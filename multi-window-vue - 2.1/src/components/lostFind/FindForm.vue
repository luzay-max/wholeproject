          ...
          <el-form-item label="物品描述" prop="description" class="form-item">
            ...
            <div class="ai-actions">
              <el-button
                ...
                :class="['ai-btn', { 'ai-btn--filled': Boolean((findForm.description || '').trim()) }]"
              >
                <el-icon><Lightning /></el-icon>
                {{ (findForm.description || '').trim() ? 'AI优化描述' : 'AI补全描述' }}
              </el-button>
              <span class="ai-hint" :class="{ 'ai-hint--active': Boolean((findForm.description || '').trim()) }">
                {{ (findForm.description || '').trim()
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const validateFindLocation = (rule, value, callback) => {
      const normalized = normalizeText(value);
      if (!normalized) return callback(new Error('请输入拾取地点'));
      if (normalized.length < 2) return callback(new Error('拾取地点至少填写2个字符'));
      if (normalized.length > 100) return callback(new Error('拾取地点不能超过100个字符'));
      if (!/[\u4e00-\u9fa5A-Za-z0-9]/.test(normalized)) {
        return callback(new Error('拾取地点格式不正确'));
      }
      if (/^\d+$/.test(compactText(normalized))) {
        return callback(new Error('拾取地点不能只填写数字'));
      }
      if (isLikelyNoise(normalized)) {
        return callback(new Error('拾取地点疑似无效，请填写具体地点'));
      }
      callback();
    };
    ...
    const rules = {
      ...
      location: [
        { validator: validateFindLocation, trigger: 'blur' }
      ],
      foundTime: [
    ...
    const normalizeImages = () => {
      if (!Array.isArray(findForm.images)) return [];
      return findForm.images.filter((url) => typeof url === 'string' && /^https?:\/\//i.test(url));
    };
    ...
    const applyAiDescription = async (suggested) => {
      ...
      const current = (findForm.description || '').trim();
    ...
    const handleAiSuggest = async () => {
      await clickGuard.run('find-ai-suggest', async () => {
        if (aiLoading.value) return;
        if (!findFormRef.value) return;

        let baseValid = true;
        try {
          await findFormRef.value.validateField(['name', 'type', 'location']);
        } catch {
          baseValid = false;
        }
        if (!baseValid) {
        ...
        try {
          const payload = {
            ...
            name: findForm.name,
            type: findForm.type,
            location: findForm.location,
            timeValue: formatTimeValue(findForm.foundTime),
            imageUrls: normalizeImages(),
            currentDescription: findForm.description || '',
    ...
    const handleSubmit = async () => {
      await clickGuard.run('find-submit', async () => {
        if (!findFormRef.value) return;

        const valid = await findFormRef.value.validate().catch(() => false);
        if (!valid) {
